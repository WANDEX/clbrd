...
struct Finfo {
  ...
  char m_fname[fname_max_len]{"clbrd_default_file_name\0"};
};
...
template <>
struct fmt::formatter<wndx::clbrd::file::Finfo> : formatter<string_view> {
  auto format(wndx::clbrd::file::Finfo const& f, format_context& ctx) const
      -> format_context::iterator;
};

/// \brief fmt formatter specialization for File.
template <>
struct fmt::formatter<wndx::clbrd::file::File> : formatter<string_view> {
  auto format(wndx::clbrd::file::File const& f, format_context& ctx) const
      -> format_context::iterator;
};
